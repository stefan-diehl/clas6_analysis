...
findElectronIDParameters::findElectronIDParameters(){   
  for (int sect=0; sect<6; sect++){
    ecSamplingFraction[sect] = new TH2I(Form("ecSamplingFraction_%d",sect),"",100,0.5,4.0,100,0.01,0.5);   // 4.5
  ...
  for (int sector=0; sector<6; sector++){
    for(int slice=0; slice<18; slice++){
      ...
      ccSlices[sector][slice] = ccThetaCorrelation[sector]->ProjectionY(name.c_str(),slice,slice+1);
        ...
        yMin_cc = 5.103 + 1.27 * slice + 0.04476 * slice * slice;
        yMax_cc = 8.72 + 2.18 * slice;

      TF1 *fitGauss_cc = new TF1("fitGauss_cc","gaus", yMin_cc, yMax_cc);  

      ccSlices[sector][slice]->Fit("fitGauss_cc","RQ");
 
      mu[slice]         = fitGauss_cc->GetParameter(1);
      sigma[slice]      = fitGauss_cc->GetParameter(2);
      muError[slice]    = fitGauss_cc->GetParError(1);
      sigmaError[slice] = fitGauss_cc->GetParError(2);
    }
...
void findElectronIDParameters::CalculateSamplingFractionCut(){
  ...
  for (int sector=0; sector<6; sector++){
    ...
    ecPol3Mu[sector]     = new TF1(Form("ecPol3Mu%d",sector),   "pol3",0.50,4.0);   // 4.5
    ecPol3Sigma[sector]  = new TF1(Form("ecPol3Sigma%d",sector),"pol3",0.50,4.0);   // 4.5
