...
public:  
 ...
 /////////////////////////////////////////////////////
    ...
    tupleWriter.addFloat("E_pim"); 
    tupleWriter.addFloat("px_pim"); 
    tupleWriter.addFloat("py_pim"); 
    tupleWriter.addFloat("pz_pim"); 
    tupleWriter.addFloat("p_ele"); 
    tupleWriter.addFloat("p_pim");
    tupleWriter.addFloat("phi_ele"); 
    tupleWriter.addFloat("phi_pim");
    tupleWriter.addFloat("theta_ele"); 
    tupleWriter.addFloat("theta_pim");
...
//////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void Loop(){
	...
	std::vector<int> pimIndices = filter->getVectorOfParticleIndices(event, -211);    // get pi- from the event

	if(!pimIndices.empty()){    // if event has pi+ (and electron)

	  int pimIndex       = pimIndices[0];
	  TLorentzVector pim = event.GetTLorentzVector(pimIndex, -211); 
	  ...
	  PhysicsEvent ev = builder.getPhysicsEvent(electron, pim);
	    ...
	    tupleWriter.setFloat("E_pim",         pim.E()); 
            tupleWriter.setFloat("px_pim",        pim.Px()); 
            tupleWriter.setFloat("py_pim",        pim.Py()); 
            tupleWriter.setFloat("pz_pim",        pim.Pz()); 
            tupleWriter.setFloat("p_ele",    	  electron.P()); 
	    tupleWriter.setFloat("p_pim",    	  pim.P()); 
	    tupleWriter.setFloat("theta_ele",     to_degrees*electron.Theta()); 
	    tupleWriter.setFloat("theta_pim",     to_degrees*pim.Theta()); 
	    tupleWriter.setFloat("phi_ele",       to_degrees*electron.Phi()); 
	    tupleWriter.setFloat("phi_pim",       to_degrees*pim.Phi());
	    tupleWriter.setFloat("dvz",           event.vz[electronIndex]-event.vz[pimIndex]); 
